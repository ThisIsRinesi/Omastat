...
if [[ -e "$plugin_dir/.git" ]]; then
  ...
  [[ -f "$plugin_dir/crates/nagori/Cargo.toml" ]] ||
...
cargo install --path "$backend_source/crates/nagori" --root "$backend_root" --locked
"$repo_root/packaging/systemd/install-user-service.sh"

if ((!git_managed)); then
  mkdir -p "$(dirname "$plugin_dir")"
  stage=$(mktemp -d "$(dirname "$plugin_dir")/.nagori-install.XXXXXX")
...
systemctl --user is-active --quiet nagori.service || fail "The tracking service is not running."

if ((with_browser)); then
  PATH="$backend_root/bin:$PATH" "$backend_source/packaging/browser-extension/install.sh"
fi
printf '\nNagori installed. Open its widget in the bar.\n'
printf 'CLI: %s/bin/nagori\nUninstall: %s/uninstall.sh\n' "$backend_root" "$repo_root"
