...
use crate::{
    ...
    insights::{
        ...
        InsightTone, Prediction, PredictionStrength, RoutineEvidence,
    },
};
use anyhow::Result;
...
pub(crate) fn predict(
    ...
    let daily_starts: Vec<usize> = starts
        .chunks(BINS)
        .map(|bins| bins.iter().map(Vec::len).sum())
        .collect();
    if daily_starts.iter().sum::<usize>() < 5 {
        return Vec::new();
    }
    ...
    for window in calendar.windows.iter().filter(|w| w.width == 4) {
        ...
        if end.timestamp() - begin.timestamp() != 3600 {
            continue;
        }
        // Reuse per-date window evidence across the overlapping cohorts.
        let mut first_starts = [None; DAYS];
        let mut window_counts = [0usize; DAYS];
        for day in 0..DAYS {
            if eligible & (1u64 << day) == 0 {
                continue;
            }
            for stamp in (window.start..window.start + window.width)
                .flat_map(|bin| starts[day * BINS + bin].iter().copied())
            {
                window_counts[day] += 1;
                first_starts[day] =
                    Some(first_starts[day].map_or(stamp, |first: i64| first.min(stamp)));
            }
        }
        for cohort in [3 + weekday, if weekday < 5 { 1 } else { 2 }, 0] {
            ...
            for (recent, mask) in [(false, cohort_mask), (true, cohort_mask & (!0u64 << 42))] {
                ...
                for day in 0..DAYS {
                    ...
                    window_starts += window_counts[day];
                    if let Some(first) = first_starts[day] {
                        matches |= 1u64 << day;
                        // Each date contributes one representative start, regardless
                        // of how many times the app was reopened that day.
                        stamps.push(first);
                    }
                }
                ...
                let all_starts: usize = (0..DAYS)
                    ...
                    .map(|day| daily_starts[day])
                    .sum();
                if window_starts * 48 < all_starts * 3 {
                    continue;
                }
                if !recent {
                    let weeks = (0..DAYS)
                ...
                candidates.push(Insight {
                    ...
                    supporting: InsightSupport {
                        share: Some(count as f64 / sample as f64),
    ...
    // Choose stable, non-overlapping routines from history before considering
    // today's clock. Expired windows must not be replaced by a later overlap.
    let mut learned: Vec<Insight> = Vec::new();
    for candidate in candidates {
        let routine = candidate.supporting.routine.as_ref().unwrap();
        if learned
            .iter()
            .any(|i| overlaps(i.supporting.routine.as_ref().unwrap(), routine))
        {
            continue;
        }
    ...
    learned
        .into_iter()
        .filter_map(|finding| {
            let end = finding.supporting.display_until?;
            let from = end - 5400;
            if now < from
                || now >= end
                || visits
                    .iter()
                    .any(|v| v.start <= now && v.end > from && v.start < end)
            {
                return None;
            }
            Some(Prediction {
                id: finding.supporting.prediction_id.clone()?,
                strength: if finding.supporting.routine.as_ref()?.status == "recent" {
                    PredictionStrength::Emerging
                } else {
                    PredictionStrength::Established
                },
                generated_at: now,
                display_from: from,
                display_until: end,
                expected_start: finding.supporting.expected_start?,
                finding,
            })
        })
        .collect()
...
mod tests {
    ...
    #[test]
    fn chronological_replay_scores_withheld_outcomes_and_expiry() {
        let first = NaiveDate::from_ymd_opt(2026, 7, 1).unwrap();
        for scenario in ["steady", "missed", "changed", "noise", "gaps"] {
            let minute = |day: i64| -> Option<i64> {
                match scenario {
                    "missed" if day >= 42 && day % 4 == 0 => None,
                    "changed" if day >= 42 => Some(22 * 60),
                    "noise" => Some(9 * 60 + (day * 137) % (13 * 60)),
                    _ => Some(20 * 60 + (day % 3 - 1) * 5),
                }
            };
            let observed = |day: i64| scenario != "gaps" || day % 5 != 0;
            let (mut hits, mut misses, mut abstentions, mut excluded, mut error) = (0, 0, 0, 0, 0);
            for day in 28..70 {
                let cutoff = first + Duration::days(day);
                let coverage = Coverage::new(
                    (0..day)
                        ...
                        .map(|d| {
                            (
                                midnight(first + Duration::days(d)).unwrap(),
                                midnight(first + Duration::days(d + 1)).unwrap(),
                            )
                        })
                        .collect(),
                );
                ...
                let visits = (0..day)
                    ...
                    .filter_map(|d| {
                        minute(d).map(|m| {
                            let start = midnight(first + Duration::days(d)).unwrap() + m * 60;
                            Visit {
                                start,
                                end: start + 1200,
                                seconds: 1200,
                    ...
                    .collect::<Vec<_>>();
                let now = midnight(cutoff).unwrap() + 19 * 3600 + 45 * 60;
                let predictions = predict(&calendar, "app", "spire", "Spire", &visits, now);
                if !observed(day) {
                    excluded += 1;
                    continue;
                }
                ...
                assert!(
                    prediction
                        ...
                        .unwrap()
                        .iter()
                        .all(|d| d < &cutoff.to_string())
                );
                let actual = minute(day).map(|m| midnight(cutoff).unwrap() + m * 60);
                let start = prediction.display_until - 3600;
                if let Some(actual) =
                    actual.filter(|actual| *actual >= start && *actual < prediction.display_until)
                {
                    hits += 1;
                    error += (actual - prediction.expected_start).abs();
                } else {
                    misses += 1;
                }
                // Re-fitting later cannot extend this occurrence after it expires.
                let expired = predict(
                    &calendar,
                    "app",
                    "spire",
                    "Spire",
                    &visits,
                    prediction.display_until,
                );
                assert!(
                    !expired
                        .iter()
                        .any(|p| p.expected_start == prediction.expected_start)
                );
            }
            println!(
                "replay {scenario}: hits={hits} misses={misses} abstentions={abstentions} excluded={excluded} mean_timing_error_seconds={}",
                error / hits.max(1)
            );
            assert_eq!(hits + misses + abstentions + excluded, 42);
            match scenario {
                "steady" => {
                    assert_eq!(hits, 42);
                    assert_eq!(misses, 0);
                    assert!(error / hits <= 600);
                }
                "missed" => assert!(misses > 0),
                "changed" => {
                    assert!(misses > 0);
                    assert!(abstentions > 0);
                }
                "noise" => assert!(abstentions >= 20),
                "gaps" => {
                    assert!(excluded > 0);
                    assert_eq!(misses, 0);
                }
