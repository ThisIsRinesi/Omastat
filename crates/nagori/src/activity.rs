...
pub struct ActivityAnalytics {
    ...
    pub predictions: Vec<crate::insights::Prediction>,
...
pub(crate) fn analyze_context(
    ...
    for ((kind, key), mut intervals) in grouped {
        ...
        if baseline_end == Local::now().date_naive() {
            let cutoff = midnight(baseline_end)?;
            let prediction_visits = visits
                .iter()
                .map(|visit| Visit {
                    seconds: intervals[visit.first..=visit.last]
                        .iter()
                        .map(|i| (i.end.min(cutoff) - i.start).max(0))
                        .sum(),
                    ..*visit
                })
                .collect::<Vec<_>>();
        ...
        a_recent
            ...
            .then(
                a.supporting
                    .expected_start
                    .map(|t| (t - end).abs())
                    .cmp(&b.supporting.expected_start.map(|t| (t - end).abs())),
            )
    ...
    // The dashboard filters by selected activity before applying its two-card limit.
    if selector.is_none() {
