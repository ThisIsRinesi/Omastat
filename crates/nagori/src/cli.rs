...
pub enum Commands {
    ...
    ExportData {
        ...
        #[arg(short, long, default_value = "nagori-data-export.json")]
        output: PathBuf,
    },
...
fn print_storage_diagnostic(diagnostic: &StorageDiagnostic) {
    ...
    match &diagnostic.schema_status {
        ...
        StorageSchemaStatus::UnknownMigration {
            ...
            println!("  Migration need: unknown until Nagori is updated");
        }
...
mod tests {
    ...
    fn parses_insights_json_after_subcommand() {
        let cli = Cli::try_parse_from([
            "nagori", "insights", "--json", "--lens", "week", "--offset", "-1",
        ])
        .unwrap();
    ...
    fn parses_summary_period_options() {
        let cli = Cli::try_parse_from([
            "nagori", "summary", "--lens", "month", "--offset", "-1", "--days", "31",
        ])
        .unwrap();
    ...
    fn summary_report_honors_days_for_historical_day() -> Result<()> {
        ...
        let db = dir.path().join("nagori.db");
    ...
    fn retired_commands_are_rejected() {
        for command in ["export", "tui"] {
            assert!(Cli::try_parse_from(["nagori", command]).is_err());
        }
    }
