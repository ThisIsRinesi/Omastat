...
fn database_lock(path: &Path) -> Result<fs::File> {
    ...
    file.try_lock().context(
        "database is in use; stop nagorid before starting another tracker or purging data",
    )?;
    Ok(file)
}
...
impl Storage {
    ...
    pub fn open_with_mode(
        ...
        if mode == StorageOpenMode::ReadOnly {
            if !path.exists() {
                anyhow::bail!(
                    "database {} does not exist; start nagorid once with write access to initialize it",
                    path.display()
                );
            }
    ...
    fn validate_schema(&self) -> Result<()> {
        let applied = self.applied_migration_versions().with_context(|| {
            format!(
                "database {} is not initialized or needs migration; start nagorid once with write access before running read-only reports",
                self.path.display()
            )
        })?;
        self.validate_known_migrations(&applied)?;
        if let Some(migration) = missing_migration(&applied) {
            anyhow::bail!(
                "database {} is not initialized or needs migration (missing migration {}: {}); start nagorid once with write access before running read-only reports",
                self.path.display(),
                migration.version,
    ...
    fn validate_known_migrations(&self, applied: &BTreeSet<i64>) -> Result<()> {
        if let Some(version) = unknown_migration_version(applied) {
            anyhow::bail!(
                "database {} has unknown schema migration {}; update Nagori before using this database",
                self.path.display(),
                version
            );
        }
...
impl Storage {
    ...
    pub fn purge_before(
        ...
        if !dry_run {
            anyhow::ensure!(!self.tracking, "stop nagorid before purging data");
            self.acquire_exclusive()?;
        }
...
mod tests {
    ...
    fn records_browser_domain_changes_by_source() {
        ...
        storage
            .record_browser_domain("nagori-zen", "zen", "github.com", 100)
            .unwrap();
        storage
            .record_browser_domain("nagori-zen", "zen", "github.com", 110)
            .unwrap();
        storage
            .record_browser_domain("nagori-zen", "zen", "chatgpt.com", 150)
            .unwrap();
    ...
    fn browser_domain_totals_overlap_focused_browser_intervals() {
        ...
        storage
            .record_browser_domain("nagori-zen", "zen", "github.com", 90)
            .unwrap();
        storage
            .record_browser_domain("nagori-zen", "zen", "chatgpt.com", 180)
            .unwrap();
        let focused = storage
    ...
    fn raw_export_includes_local_timestamps_and_system_gaps() {
        ...
        storage
            .record_browser_domain("nagori-zen", "zen", "github.com", 120)
            .unwrap();
        storage
            .record_browser_domain("nagori-zen", "zen", "chatgpt.com", 170)
            .unwrap();
    ...
    fn purge_before_deletes_old_rows_and_trims_overlaps() {
        ...
        storage
            ...
            .unwrap();
        storage
            .record_browser_domain("nagori-zen", "zen", "github.com", 180)
            .unwrap();
        storage
            .record_browser_domain("nagori-zen", "zen", "github.com", 250)
            .unwrap();
        storage
            .record_browser_domain("nagori-zen", "zen", "chatgpt.com", 300)
            .unwrap();
