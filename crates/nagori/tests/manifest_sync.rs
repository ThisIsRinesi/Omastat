...
const PLUGIN_ID: &str = "local.nagori";
const ROOT_ENTRYPOINT: &str = "packaging/omarchy/nagori/BarWidget.qml";
...
fn omarchy_manifests_stay_synchronized() {
    ...
    let packaged_manifest_path = workspace.join("packaging/omarchy/nagori/manifest.json");
    ...
    assert_entrypoint_exists(
        &workspace.join("packaging/omarchy/nagori"),
...
fn packaged_plugin_data_matches_omarchy_bar_widget_contract() {
    let workspace = workspace_root();
    let plugin_dir = workspace.join("packaging/omarchy/nagori");
...
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("crate should live under crates/nagori")
        .to_path_buf()
}
