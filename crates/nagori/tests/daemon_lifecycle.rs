...
root = Path(os.environ['NAGORI_TEST_ROOT'])
...
"#,
    ...
    fn command(&self, binary: &str) -> Command {
        ...
        cmd.args(["--config", "/dev/null", "--database"])
            ...
            .env("NAGORI_TEST_ROOT", self.root.path())
    ...
    fn daemon(&self) -> Daemon {
        Daemon(
            self.command(env!("CARGO_BIN_EXE_nagorid"))
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn()
                .unwrap(),
...
fn disconnected_daemon_rejects_competitors_and_purge_but_allows_preview_and_shutdown() {
    ...
    let purge = desktop
        .command(env!("CARGO_BIN_EXE_nagori"))
        .args(["purge", "--all", "--confirm"])
        .output()
        .unwrap();
    assert!(!purge.status.success());
    assert!(String::from_utf8_lossy(&purge.stderr).contains("stop nagorid"));
    let preview = desktop
        .command(env!("CARGO_BIN_EXE_nagori"))
