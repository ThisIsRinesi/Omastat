...
function weekTrendSummary(daily, todayKey) {
  ...
  if (stats.bestDaySeconds > 0) parts.push("Peak " + stats.bestDayLabel + " " + fmt(stats.bestDaySeconds))
...
function monthDefaultText(cells, weekly) {
  ...
  return (weekly ? "Most-used week " : "Most-used day ") + String(best.label || best.date || "") + ": " + fmt(best.seconds)
}
...
// Spread the first visible findings across activities without changing their claims.
function diverseInsights(items) {
  var first = [], remaining = [], seen = {}
  for (var i = 0; i < (items || []).length; i++) {
    var support = items[i].supporting || {}
    var key = String(support.activity_kind || "app") + ":" + String(support.activity_key || support.app_class || items[i].kind || "")
    if (seen[key]) remaining.push(items[i])
    else { seen[key] = true; first.push(items[i]) }
  }
  return first.concat(remaining)
}

// Formatting only: the engine decides which findings have enough support.
...
function insightDateRange(start, end) {
  var first = insightDate(start), last = insightDate(end)
  if (String(start).slice(0, 4) === String(end).slice(0, 4)) first = first.replace(/, \d{4}$/, "")
  return first + "–" + last
}
...
function eligibleInsights(items) {
...
function widgetInsights(items) { return diverseInsights(eligibleInsights(items)) }
...
function dashboardInsights(items, predictions) {
  ...
  for (var i = 0; i < (predictions || []).length; i++) {
    var p = predictions[i]
    var ps = p.supporting || {}
    var key = String(ps.activity_kind) + ":" + String(ps.activity_key)
  ...
  var ranked = eligibleInsights(items).filter(function(item) {
    var s = item.supporting || {}
    var matching = keys[String(s.activity_kind) + ":" + String(s.activity_key)]
    if (item.kind !== "app-routine" || !matching) return true
    return !matching.some(function(prediction) {
      if (!s.routine || !(prediction.supporting || {}).routine) return true
      return routineWindows(item).some(function(a) {
        return routineWindows(prediction).some(function(b) {
          return a.start < b.start + b.width && b.start < a.start + a.width
        })
      })
    })
  ...
  return diverseInsights(ranked.map(function(entry) { return entry.item }))
}
...
function insightQualifier(item) {
  ...
  if (item && item.kind === "upcoming-activity") return ""
  if (item && item.confidence === "low") return "Recent pattern"
...
function insightPresentation(item) {
  ...
  return {
    ...
    frequency: item.kind === "upcoming-activity" && total > 0 ? count + " of " + total + " tracked opportunities" : routine && total > 0 ? Math.round(count / total * 100) + "% of tracked " + (support.weekday_label ? support.weekday_label + "s" : "days") : "",
...
function visiblePredictions(items, nowSeconds, selectedKind, selectedKey) {
  return (items || []).filter(function(item) {
    var s = item.supporting || {}
    var until = Number(item.display_until || s.display_until || 0)
    var from = Number(item.display_from || until - 5400)
    var generated = Number(item.generated_at || s.generated_at || 0)
    return until > nowSeconds && from <= nowSeconds
      && (!generated || generated <= nowSeconds && nowSeconds - generated <= 120)
