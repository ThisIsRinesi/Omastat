...
ColumnLayout {
  ...
  function finish() {
    ...
    report = null
    if (exitCode !== 0) {
      errorText = "Status unavailable. Run nagori doctor to check the tracker and database."
      return
    }
    try {
  ...
  Process {
    id: statusProcess
    command: ["bash", "-c", "PATH=\"$HOME/.cargo/bin:$HOME/.local/bin:$PATH\"; exec nagori tracking-status"]
