...
Ui.BarWidget {
  ...
  Process {
    ...
    stderr: StdioCollector { waitForEnd: true; onStreamFinished: if (text.trim()) console.warn("nagori detail", text.trim()) }
  }
  ...
  function refreshDetail(force) {
    ...
    detailError = ""
    detailRunning = true
    detailProcess.command = ["bash", "-c", 'PATH="$HOME/.cargo/bin:$HOME/.local/bin:$PATH"; exec nagori activity-detail "$@"', "nagori",
  ...
  property string tooltip: "Nagori"
  ...
  Timer {
    interval: (root.opened ? Math.min(60, Math.max(15, Number(root.setting("refreshIntervalSec", 60)))) : Math.max(15, Number(root.setting("refreshIntervalSec", 60)))) * 1000
    running: true
    repeat: true
  ...
  Process {
    ...
    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("nagori", text.trim())
    }
  }
  ...
  function summaryCommand(lens, offset) {
    ...
    return "nagori summary --lens " + safeLens
      + " --offset " + safeOffset
  ...
  function widgetSummaryCommand(lens, offset) {
    var safeLens = normalizedLens(lens)
    var safeOffset = safeLens === "life" ? 0 : Math.min(0, Math.floor(Number(offset) || 0))
    return "nagori widget-summary --lens " + safeLens
      + " --offset " + safeOffset
  }
  ...
  function normalizeWidgetSummary(parsed) {
    ...
    return {
      ...
      tooltip: String(object.tooltip || "Nagori"),
  ...
  function normalizeInsights(list) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      output.push({
        ...
        expected_start: item.expected_start,
        kind: kind,
