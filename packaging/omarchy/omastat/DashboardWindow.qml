import QtQuick
// Quickshell supplies the concrete PanelWindow implementation at runtime.
...
PanelWindow {
  ...
  readonly property alias contentContainer: contentHolder
  ...
  visible: open || (islandStyle ? expansion > 0 : card.opacity > 0) || popoutSwitching
  color: "transparent"
  ...
  Ui.BorderSurface {
    id: card
    x: root.islandStyle ? root.cardOrigin.x + (root.contentWidth - width) / 2 : root.cardOrigin.x
    y: root.topIsland ? Math.max(0, (root.bar && "islandHeight" in root.bar ? root.bar.islandHeight : root.barH) - 1) : root.cardOrigin.y
    width: root.islandStyle ? Math.min(root.contentWidth, Style.space(180)) + (root.contentWidth - Math.min(root.contentWidth, Style.space(180))) * root.expansion : root.contentWidth
    height: root.islandStyle ? Math.max(1, root.contentHeight * root.expansion) : root.contentHeight
    ...
    Shape {
      ...
      ShapePath {
        ...
        fillColor: "#000000"
        startX: 0; startY: 0
        PathCubic { x: islandShape.r; y: islandShape.r; control1X: islandShape.r; control1Y: 0; control2X: islandShape.r; control2Y: 0 }
        PathLine { x: islandShape.r; y: islandShape.height - islandShape.r }
        PathQuad { x: islandShape.r * 2; y: islandShape.height; controlX: islandShape.r; controlY: islandShape.height }
        PathLine { x: islandShape.width - islandShape.r * 2; y: islandShape.height }
        PathQuad { x: islandShape.width - islandShape.r; y: islandShape.height - islandShape.r; controlX: islandShape.width - islandShape.r; controlY: islandShape.height }
        PathLine { x: islandShape.width - islandShape.r; y: islandShape.r }
        PathCubic { x: islandShape.width; y: 0; control1X: islandShape.width - islandShape.r; control1Y: 0; control2X: islandShape.width - islandShape.r; control2Y: 0 }
        PathLine { x: 0; y: 0 }
      }
    }
    ...
    Item {
      ...
      opacity: root.islandStyle ? Math.max(0, (root.expansion - 0.65) / 0.35) : (root.popoutSwitching ? (root.open ? 1.0 : 0) : 1.0)

      enabled: root.open && (!root.islandStyle || root.expansion > 0.98)
      Behavior on opacity {
        enabled: !root.islandStyle && !root.reduceMotion && root.popoutSwitching
        NumberAnimation { duration: 140; easing.type: Easing.OutCubic }
      }
    }
  }
