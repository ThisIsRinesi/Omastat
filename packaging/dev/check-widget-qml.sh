...
qml_check_mode="${OMASTAT_QML_CHECK_MODE:-desktop}"
case "$qml_check_mode" in
  desktop|portable) ;;
  *) echo "Invalid OMASTAT_QML_CHECK_MODE: $qml_check_mode (use desktop or portable)" >&2; exit 2 ;;
esac
...
"$qml_lint" -I "$qml_imports" --unqualified disable --missing-property disable \
  ...
  "${runtime_flags[@]}" --max-warnings 0 "${qml_files[@]}" "$widget_dir/Model.js"
