...
Nagori or restart desktop services. Missing required tools fail the check;
...
of the backend binaries. CI sets `NAGORI_QML_CHECK_MODE=portable` because its Qt installation lacks
...
- Qt 6 lint and format tools (CI pins 6.11.2, matching local validation).
  `NAGORI_QT_BIN_DIR` can select their directory; otherwise `QT_ROOT_DIR/bin`,
...
- Omarchy Quattro's `omarchy-plugin-remove`, `omarchy-plugin-update`, and
  ...
  `/usr/share/omarchy/bin`; set `NAGORI_TEST_OMARCHY_BIN` to another checkout's
...
`nagori tracking-status` returns JSON from a read-only database connection.
...
`nagori doctor` for detailed diagnostics. Current status is independent of the
...
```sh
python3 packaging/dev/test-widget-layout.py --output /tmp/nagori-layout-audit
```
